 ...
 *------------------------------------*/
  ...
  function getTokenCharCode(token) {
    ...
    if (token.length > 1 && token.charAt(0) === '\\') {
      ...
      if (/^c[A-Za-z]$/.test(t)) {
        return (
          'ABCDEFGHIJKLMNOPQRSTUVWXYZ'.indexOf(t.charAt(1).toUpperCase()) + 1
        );
      }
  ...
  function parseCharClass(value) {
    ...
    output += parts.closing
      ? parts.opening
      : errorize(parts.opening, error.UNCLOSED_CLASS);
      ...
      if (m.charAt(0) === '\\') {
        ...
        if (lastToken.rangeable) {
          ...
          if (nextToken) {
            ...
            if (
              (!isNaN(nextTokenCharCode) &&
                lastToken.charCode > nextTokenCharCode) ||
    ...
    while ((match = regexToken.exec(pattern))) {
      ...
      if (char0 === '[') {
        ...
        if (!openGroups.length) {
          ...
          lastToken = {
            quantifiable: !/^[=!]/.test(
              openGroups[openGroups.length - 1].opening.charAt(2)
            ),
        ...
        if (lastToken.quantifiable) {
          ...
          if (
            interval &&
            (+interval[1] > 65535 || (interval[2] && +interval[2] > 65535))
          ) {
        ...
        if (
          ...
          output += openGroups.length
            ? groupize('|', groupStyleDepth)
            : '<b>|</b>';
        }
        lastToken = {
          quantifiable: false,
