...
export const movers = {
  ...
  mouseDown_handler: (event, props) => {
    ...
    element.addEventListener('mousemove', event =>
      drop.mouseMove_handler(event, props)
    );
    ...
    if (data.action === 'move') {
      const previousSibling = () => {
        ...
        if (move.source_address[move.source_address.length - 1] > 0) {
          _source_address.splice(
            move.source_address.length - 1,
            1,
            _source_address[move.source_address.length - 1] - 1
          );
      ...
      let sectionEntity = address.byPath(
        props.form,
        destination_address.slice(0, destination_address.length - 1)
      );
      console.log(sectionEntity, sectionEntity.children());
      let sectionAddress = destination_address.slice(
        0,
        destination_address.length - 1
      );
      ...
      const firstInRow = entity_address => {
        ...
        const _entity_address =
          entity_address.slice(
            entity_address.length - 1,
            entity_address.length + 1
          ) - 1;
        var runningTotal = 0;
      ...
      /**mutate entity itself */
      console.log(
        address.byPath(props.form, data.address).UUID(),
        props.model.UUID()
      );
      ...
      if (arraysEqual(source_address, destination_address)) {
        ...
        console.log(move.source_address, {
          prepend: firstInRow(destination_address)
            ? move.init_prepend + grid()
            : 0,
          append: move.init_append - grid(),
        });
