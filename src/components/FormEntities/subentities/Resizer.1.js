    ...
    if (resize.grids !== grid() && event.pageX !== 0) {
      ...
      if (!true) {
        ...
        if (locEntity[1].type() === 'FormSection') {
          ...
          resize.init_children === null ? (resize.init_children = locEntity[1].children()) : null;

          // map through children starting here
          console.log('here: ', locEntity[1].children().length);
          /** detirmine if there are children in section */
          if (locEntity[1].children().length > 0) {
            const total = (prePromptWidth, prepend, width, append, postPromptWidth) =>
            ...
            const _children = resize.init_children.map(child =>
              Object.assign({}, child.properties(), {
                total: total(
                  child.prePromptWidth ? child.prePromptWidth() : 0,
                  child.prepend(),
                  child.width(),
                  child.append(),
                  child.postPromptWidth ? child.postPromptWidth() : 0
                ),
                row: null,
                index: null,
              })
            ...
            const reducer = (accumulator, currentValue, currentIndex) => {
              ...
              if (accumulator)
                if (_children.length - 1 === currentIndex) {
                  ...
                  console.log('return last entity in section');

                  const occupiedColumnsInRow = (entities, prop) =>
                    entities.reduce((a, b) => {
                      console.log(131, a, b.row);
                  ...
                  if (
                    ...
                    console.log('can add other entity');
                    console.log(
                      'testhere: ',
                    ...
                    accumulator[accumulator.length] = Object.assign({}, _children[currentIndex], {
                      append:
                        _children[currentIndex].append + (sectionWidth - occupiedColumnsInRow(_children, 'total')),
                    });
                    return accumulator;
                  }
                }
              ...
              if (
                ...
                console.log('accumulator: ', accumulator[accumulator.length - 1]);

                accumulator[accumulator.length - 1] = Object.assign({}, _children[currentIndex - 1], {
                  index: 0,
                  row: counter,
                });
                ...
                accumulator[accumulator.length - 1] = Object.assign({}, accumulator[accumulator.length - 1], {
                  total:
                    ...
                    (sectionWidth - occupiedColumnsInRow(accumulator, 'total')),
