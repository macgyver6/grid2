...
const FormComponent = props => {
  ...
  const drop_handler = event => {
    ...
    const appendGrids = round(
      (event.clientX -
        document.getElementById('form').getBoundingClientRect().left) /
        bgrndGrdWidth,
      0
    );
    if (data && data.action === 'addEntity') {
      ...
      let entityToAdd = address.resurrectEntity(
        Object.assign({}, data.model, {
          ...
          append:
            props.form.children()[props.activeTab].width() -
            appendGrids -
            defaultPropsFE[data.model.type].width,
        })
      );
      const whereToAdd = [
        props.activeTab,
        props.form.children()[props.activeTab].children().length,
      ];
    ...
    if (data && data.action === 'move') {
      ...
      const appendGridsEOffset = round(
        (event.clientX -
          document.getElementById('form').getBoundingClientRect().left -
          offsetE1) /
          bgrndGrdWidth,
